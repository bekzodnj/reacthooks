      ...
      <section className='overflow-x-scroll p-2'>
        ...
        <div className='mb-4'>
          <p>
            ...
            that it does not re-created each time when parent state changes.
...
const WithCallback = ({ checked }) => {
  ...
  const infoDescriptionBox = checked ? (
    <p className={'font-normal mb-5 text-gray-400'}>
      ...
      fibonacci(). So child does not re-render when receiving the same props.
    </p>
  ) : (
    <p className={'font-normal mb-5 text-gray-400'}>
      <span role='img' aria-label='bulb'>
  ...
  return (
    <div className={`bg-slate-900 text-white p-2`}>
      ...
      <p className='mb-3'>
        {' '}
        Parent component rendered at: {time.toLocaleTimeString('en-us')}
      </p>
      <button
  ...
  return (
    <div className='border border-blue-900'>
      ...
      <span className='inline-block italic text-gray-300'>
        Child component last re-rendered at:{' '}
...
const WithoutUseCallback = ({ fibonacci, num }) => {
  return (
    <div className='border border-blue-900'>
      <h3 className='font-mono'>
        The value of fibonacci({num}) = {fibonacci(num)}
